defmodule Apiv2.WeighticketController do
  ...
  def index(conn, _params) do
    weightickets = Repo.all(Weighticket) |> Repo.preload(@preload_fields)
  ...
  def create(conn, %{"weighticket" => weighticket_params}) do
    ...
    if changeset.valid? do
      weighticket = Repo.insert(changeset) |> Repo.preload(@preload_fields)
  ...
  def update(conn, %{"id" => id, "weighticket" => weighticket_params}) do
    ...
    if changeset.valid? do
      weighticket = Repo.update(changeset) |> Repo.preload(@preload_fields)
      render(conn, "show.json", weighticket: weighticket)
    else
      conn
      |> put_status(:unprocessable_entity)
